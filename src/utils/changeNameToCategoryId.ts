...
export const changeNameToCategoryId = (categoryName: string): number => {
	let type = const_categoryTypes.interest_mortage;
	if (categoryName === 'Interest') return (type = const_categoryTypes.interest_mortage);
...
export const changeCategoryIdToName = (categoryType: number): string => {
	...
	if (categoryType === const_categoryTypes.interest_mortage) return (categoryName = 'Interest');
